...
public class PhotoTools {
     ...
     * @return
     */
    public static List<PhotoFolderInfo> getAllPhotoFolder(Context context, Map<String, PhotoInfo> selectPhotoMap, boolean isLoadVideo) {
            ...
            if (isLoadVideo) {
                ...
                if (dataVideo != null) {
                    while (dataVideo.moveToNext()) {
                        ...
                        Log.i("PhotoTools", "duration4:" + duration);
                        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                        Date date = new Date(videoAddTime * 1000);
                        Log.i("PhotoTools", "转换后时间：" + sdf.format(date) + "   视频日期：" + videoAddTime + "   视频路径：" + videoPath);

                        //判断视频大小 大于200MB不添加
                        if (!formetFileSize(videoSize)) {
                            Log.i("PhotoTools", "videoName:" + videoName);
                            continue;
                        }
